...
- **Validação e Download de Certificados (`certificates.service.ts`):**
  - Correção na busca de certificados em `validateCertificate` e `generatePdfBuffer` para suporte a busca insensível a maiúsculas/minúsculas (_case-insensitive_) e aceitação de código hash ou ID único.
